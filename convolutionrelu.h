...
loopImageBlockX:
    for (unsigned int i = 0; i < 2; ++i)
        {
        layerOutBlock_t rawBlock_Old = Old.Y[cI * ((sizeX - BLOCK_WIDTH) / BLOCK_HEIGHT) + xI / BLOCK_HEIGHT + i];
        layerOutBlock_t rawBlock_New = New.Y[cI * ((sizeX - BLOCK_WIDTH) / BLOCK_HEIGHT) + xI / BLOCK_HEIGHT + i];
